...
#
# The tone is generated by a DDS in the PL -- see hdl/dac_tone_src.v -- and
# its frequency is written to axi_gpio_freq at 0xA4020000.
#
...
# Phase increment for dac_tone_src, one 32-bit output word. The reset value
# is the tone the fixed table it replaced produced, so an image that never
# writes this register behaves as before: 0x08000000 / 2^32 * 3932.16 MHz.
set gpio_freq [create_bd_cell -type ip -vlnv xilinx.com:ip:axi_gpio:2.0 axi_gpio_freq]
# Channel 2 in is dac_tone_src's readback: the accumulator, tready and the
# reset of the 491.52 MHz domain. Without it a stalled datapath and a
# mistuned one look identical from software, which is what made the DDS
# bring-up expensive.
set_property -dict [list \
    CONFIG.C_IS_DUAL {1} \
    CONFIG.C_GPIO_WIDTH {32}  CONFIG.C_ALL_OUTPUTS {1} \
    CONFIG.C_GPIO2_WIDTH {32} CONFIG.C_ALL_INPUTS_2 {1} \
    CONFIG.C_DOUT_DEFAULT {0x08000000} \
    CONFIG.C_INTERRUPT_PRESENT {0} \
] $gpio_freq

# Step 7: Control path -- SmartConnect fanning FPD_AXI_PL out to 4 slaves.
...
set_property -dict [list CONFIG.NUM_MI {4} CONFIG.NUM_SI {1} CONFIG.NUM_CLKS {1}] $axi_smc
...
connect_bd_net [get_bd_pins ps_wizard_0/pl0_ref_clk] \
    ...
    [get_bd_pins axi_gpio_freq/s_axi_aclk] \
...
connect_bd_net [get_bd_pins rst_pl0/peripheral_aresetn] \
    ...
    [get_bd_pins axi_gpio_freq/s_axi_aresetn] \
...
connect_bd_intf_net [get_bd_intf_pins axi_smc/M03_AXI] [get_bd_intf_pins axi_gpio_freq/S_AXI]
...
connect_bd_net [get_bd_pins axi_gpio_freq/gpio_io_o]     [get_bd_pins dac_tone_src_0/freq_word]
connect_bd_net [get_bd_pins dac_tone_src_0/dbg]          [get_bd_pins axi_gpio_freq/gpio2_io_i]
...
set pl_segments {
    ...
    {axi_gpio_freq/S_AXI/Reg         0xA4020000 0x00010000}
