...
#
# Build PDI and export hardware platform for VRK160 rfloop overlay.
...
#
# A failure here is FATAL. pr_verify is the only thing that proves this
# overlay is a valid segmented child of the golden that produced the running
# boot PDI; an overlay that fails it will not load correctly at runtime.
# Continuing past it produces an rfloop.pdi that looks fine and a build that
# claims success, which is worse than no artefact at all.
set golden_dcp "../golden/golden_routed.dcp"
if {![file exists $golden_dcp]} {
    puts "ERROR: Golden routed DCP not found at $golden_dcp"
    puts "       Cannot verify overlay compatibility. Build the golden first."
    exit 1
}
...
if {[llength $overlay_dcps] == 0} {
    puts "ERROR: No routed DCP found for the overlay; cannot run pr_verify."
    exit 1
}
...
if {$result != 0} {
    puts ""
    puts "ERROR: pr_verify FAILED -- this overlay is NOT compatible with the"
...
puts ""
puts "VRK160 rfloop overlay built successfully (Segmented Configuration):"
