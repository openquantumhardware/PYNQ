...
Same design as `boards/VRK160/sgloop`, driven by Hog instead of the Makefile.
...
Paths inside `list/` are relative to it, not to the git root.
    ...
    boards/VRK160/
      ...
      Top/vrk160/sgloop/ <- this project
      golden/           <- golden reference, must be built first
      sgloop/           <- the design sources
...
## Building
    ...
    cd boards/VRK160
    ./Hog/Do CREATE   Top/vrk160/sgloop
    ./Hog/Do WORKFLOW Top/vrk160/sgloop
    make -C sgloop dtbo                   # needs sdtgen from Vitis on PATH

## Why Top/vrk160/sgloop and not Top/rfloop
...
## What runs where
    ...
    proj.tcl                  sources sgloop/sgloop.tcl, builds the BD wrapper
    ...
    post-bitstream.tcl        pr_verify against the golden, then writes sgloop.xsa
...
* `list/src.src` is empty on purpose. `sgloop.tcl` adds its own HDL and
  ...
  Moving the file lists out of `sgloop.tcl` and into `list/` is the follow-up.
